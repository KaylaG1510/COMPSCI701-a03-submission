...
public class BoardHandler {
    ...
    private Board _currentBoardState;
    ...
    public boolean startPlayerTurn(boolean isPlayerOneTurn, int selectedHouse, IO io) {
        ...
        int numSeedsToSow = _currentBoardState.getPit(startHouseIndex).getNumSeeds();
    ...
    private int sowSeeds(int numSeedsToSow, int startHouseIndex, boolean isPlayerOneTurn) {
        ...
        while(movesMade <= numSeedsToSow) {
            ...
            _currentBoardState.getPit(startHouseIndex + movesMade).addSeeds(1);
            movesMade++;
    ...
    private boolean p1CanCapture(int currentPos, boolean isPlayerOneTurn) {
        boolean p1LandsOwnEmptyHouse = (currentPos < P1STOREINDEX) && _currentBoardState.getPit(currentPos).getNumSeeds() == 1;
        boolean oppHouseNotEmpty = _currentBoardState.getPit(getOpposingHouseIndex(currentPos)).getNumSeeds() != 0;
    ...
    private boolean p2CanCapture(int currentPos, boolean isPlayerOneTurn) {
        boolean p2LandsOwnEmptyHouse = (currentPos > P1STOREINDEX) && _currentBoardState.getPit(currentPos).getNumSeeds() == 1;
        boolean oppHouseNotEmpty = _currentBoardState.getPit(getOpposingHouseIndex(currentPos)).getNumSeeds() != 0;
    ...
    private void captureSeeds(int currentPos, boolean isPlayerOneTurn) {
        int oppSeeds = _currentBoardState.getPit(getOpposingHouseIndex(currentPos)).captureSeeds();
        int ownSeed = _currentBoardState.getPit(currentPos).captureSeeds();
        if(isPlayerOneTurn)
            _currentBoardState.getPit(P1STOREINDEX).addSeeds(oppSeeds + ownSeed);
        else
            _currentBoardState.getPit(P2STOREINDEX).addSeeds(oppSeeds + ownSeed);
    }
    ...
    public boolean checkBoardEmpty(boolean isPlayerOneTurn) {
        ...
        for(int i = 0; i < NUMHOUSES; i++)   {
            if(i != P1STOREINDEX)
                p1seedsRemaining += _playerOne.getPlayerPits().get(i).getNumSeeds();
            if(i != P2STOREINDEX)
                p2seedsRemaining += _playerTwo.getPlayerPits().get(i).getNumSeeds();
        }
     ...
     * @return
     */
    public Board saveBoardState(boolean isP1Turn) {
        ArrayList<Pit> copyOfCurrentBoard = new ArrayList<>();
        _playerOne = new Player();
        _playerTwo = new Player();

        for(int i = 0; i <= NUMHOUSES; i++) {
            _playerOne.getPlayerPits().get(i).clonePit(_currentBoardState.getPit(i).getNumSeeds());
            _playerTwo.getPlayerPits().get(i).clonePit(_currentBoardState.getPit(i + NUMHOUSES  + 1).getNumSeeds());
        }
        copyOfCurrentBoard.addAll(_playerOne.getPlayerPits());
        copyOfCurrentBoard.addAll(_playerTwo.getPlayerPits());

        return new Board(copyOfCurrentBoard, isP1Turn);
    }
     ...
     * @return
     */
    public boolean loadRecentBoardState(Board recentlySavedBoard) {
