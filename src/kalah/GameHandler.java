...
/**
 * RECEIVER CLASS for Command Design
 * This class controls the state of the Game, storing all variables that describe the curernt state of the game
 * as well as containing a BoardHandler which controls the State of the Board.
 * @author mpie374
 */
public class GameHandler {
    ...
    private SavedBoardStates _savedBoardStates;
    private GameActionInvoker _gameActionInvoker;

    GameHandler() {
        _gameActionInvoker = new GameActionInvoker();
    }
     ...
     */
    public Board initialiseNewGameState() {
        _boardHandler = new BoardHandler();
        _savedBoardStates = new SavedBoardStates();
        _playing = true;
    ...
    private int validatePlayerInput(String playerInput, IO io) {
        ...
        if(isInputInt(playerInput)) {
            ...
            _gameActionInvoker.setCommand(new NewGameCommand(this, io));
        } else if(playerInput.equalsIgnoreCase("S")) { //Save Game
            _gameActionInvoker.setCommand(new SaveGameCommand(this, io));
        } else if(playerInput.equalsIgnoreCase("L")) { //Load Game
            _gameActionInvoker.setCommand(new LoadGameCommand(this, io));
        } else if(playerInput.equalsIgnoreCase("Q")) { //Quit Game
            _gameActionInvoker.setCommand(new QuitGameCommand(this, io));
